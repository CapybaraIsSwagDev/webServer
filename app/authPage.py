...
def login():
    ...
    if auth.validToken(request.cookies.get('sessionToken')): # Find Session based on Client Token
        dest = request.args.get("dest") # Cookie found! Redirecting.
...
@authPage.route('/logout', methods=['POST'])
def logOut():
